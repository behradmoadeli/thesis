...
\begin{equation} \label{eq:2_observer_continuous}
    ...
    \end{aligned}
\end{equation}
where $\underline{\hat{x}}(\zeta, t)$ is the reconstructed state of the original system and $\mathfrak{L}_c$ is the continuous-time observer gain. By subtracting the observer dynamics from the original system dynamics, the error dynamics $e(\zeta,t$) are obtained as shown in \eqref{eq:2_observer_error_continuous}.
...
\begin{figure}[!htbp]
    \centering
    \includesvg[inkscapelatex=false, height=0.23\textwidth, keepaspectratio]{papers/2_conferences/figures/obs_lambda.svg}
