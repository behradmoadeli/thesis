...
As mentioned earlier, the state space $X \subset \mathrm{L}^2([0,1]; \mathbb{R}^4)$ is infinite-dimensional, and the discrete-time operators $A_d, B_d, C_d, D_d$ are those derived via the Cayley--Tustin discretization of the continuous-time system in equation~\eqref{eq:3_discrete_model}. Building on top of the estimation model in \Citeauthor{xie2022constrained}'s work\autocite{xie2022constrained}, the key difference here is the presence of the input term $B_d u_k$, which is essential in the control-integrated setting of our application. In Xie's formulation, the term $B_d w_k$ corresponds to what is denoted as $\mathcal{N}_d w_k$ in our model.
